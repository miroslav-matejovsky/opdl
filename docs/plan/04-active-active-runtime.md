# Stage 4: Active-active runtime

Estimate: 6 person-days.

Complexity: high.

The additional day covers instance-owned storage composition and sibling
...
3. Refactor runtime composition only as far as needed to make instance identity
   ...
   registration service and reconciler, one local storage writer, one optional
   sibling storage reader and synchronizer, and one HTTP server.
4. Start both processes independently. Do not have primary spawn secondary or
   secondary monitor primary. Process supervision belongs to Stage 7.
...
7. Preserve strict startup order per process: recorder, selected fabric member,
   local stores, initial sibling synchronization, registration projection
   rebuild and reconciliation, then selected HTTP listener.
8. Preserve reverse shutdown order and graceful HTTP drain per process. Stopping
   one process must stop its synchronizer and must not signal, write, or close
   resources owned by its sibling.
...
10. Update `task run` and scenario helpers to pass an explicit instance. A helper
    ...
    a machine.
11. Resolve the persistent storage root from runtime configuration and the
    selected instance identity. Primary and secondary paths must be stable,
    distinct, outside versioned package content, and visible read-only through
    the sibling capability. Log effective paths without logging stored content.
...
- Both reconcilers run and both fabric lifecycle logs identify the correct
  instance.
- Both local synchronizers run independently and converge their stores while
  Olric is stopped or unavailable.
...
- Cancellation and partial-start cleanup remain covered for each selected
  instance.
- A storage or startup synchronization failure prevents only that process from
  becoming ready and does not close its running sibling's resources.
...
and the SDK in Stage 6 will distribute traffic.
...
- Each runtime writes only its selected local store and reads both configured
  local stores through the approved capabilities.
