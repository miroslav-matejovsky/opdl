...
- Keep one ordered journal per site. Journal order is the common ordering used
  by every projection, including registration conflict resolution.
...
- Start with one embedded NATS server per OPDL node and derive its site cluster
  peers from the deployment descriptor. Stage 1 must confirm the storage
  replication rule, especially for two-node sites.
