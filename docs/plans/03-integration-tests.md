...
instances end-to-end over one lease file** through whole lifecycles. That is
exactly the shape a machine runs, and it is cheap to run in-process: two
`ManageOwnership` calls, two `recordingRuntime`s, one temp lease file, millisecond
...
2. **Failover** — Primary is Active, then "dies" (its ManageOwnership ctx is killed
...
3. **No promotion while peer healthy** — lease artificially lapsed but probe
   ...
   activates.
4. **Failback with stabilization** — after failover, the Primary instance is
   restarted (new ManageOwnership on the same lease). Standby stays Active through the
   ...
   `passive start` after `active stop` within the same ManageOwnership call).
...
7. **In-place cycling** — one Standby ManageOwnership call goes
...
- Follow the house rules: no `require.*` inside `require.Eventually` condition
  functions; drain every ManageOwnership goroutine before the test returns so
