...
//
// NewHandler wires the registration service into exact request, status, and list
// routes. The package owns strict request decoding and response encoding; the
// caller owns the server's lifecycle. Response body types come from platform/api
// so the served JSON and the published contract stay identical.
//
