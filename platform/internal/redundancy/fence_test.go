...
func TestFencePathIsRoleIndependent(t *testing.T) {
	t.Parallel()

	dir := t.TempDir()
	path := redundancy.FencePath(dir, "proj", "env", "site", "machine")

	// Both process roles contend for one machine lock.
...
func TestOpenFenceRejectsInvalidRole(t *testing.T) {
	...
	_, err := redundancy.OpenFence(path, redundancy.ProcessRole("other"))
	require.Error(t, err)
}
...
func TestFenceAcquireReleaseCycle(t *testing.T) {
	...
	path := redundancy.FencePath(t.TempDir(), "p", "e", "s", "m")
	f, err := redundancy.OpenFence(path, redundancy.RolePrimary)
...
// TestFenceIsExclusive checks two processes never hold the machine fence together:
...
func TestFenceIsExclusive(t *testing.T) {
	...
	a, err := redundancy.OpenFence(path, redundancy.RolePrimary)
	require.NoError(t, err)
	b, err := redundancy.OpenFence(path, redundancy.RoleStandby)
	require.NoError(t, err)
...
func TestFenceAcquireStopsOnCanceledContext(t *testing.T) {
	...
	holder, err := redundancy.OpenFence(path, redundancy.RolePrimary)
	require.NoError(t, err)
	...
	standby, err := redundancy.OpenFence(path, redundancy.RoleStandby)
	require.NoError(t, err)
