...
standby slot is not disabled, an optional standby. Only the local OS-fence owner
opens storage, handlers, readiness publication, and HTTP. The other process
maintains a client-only projection and promotes after the fence is released.
...
whichever process holds the fence binds them; the fence is released only after
